...
class Devices(BaseEndpoint):
    ...
    def get(self, 
        ...
        if 'saasfilter' in params:
            saas_val = params.pop('saasfilter')
            if isinstance(saas_val, (list, tuple)):
                for i, val in enumerate(saas_val):
                    params[f'saasfilter[{i}]'] = val
            else:
                params['saasfilter'] = saas_val
