...
def main():
    ...
    try:
        ...
        logger.info(
            "This demonstrates the fixed authentication with sources=true parameter"
        )
        ...
        pprint(
            domains[:10] if len(domains) > 10 else domains
        )  # Show only first 10 if there are many

        # Example 3: Get detailed information about watched domains
        logger.info("\nGetting detailed information about watched domains...")
        logger.info(
            "This demonstrates the fixed authentication with fulldetails=true parameter"
        )
        detailed_domains = client.intelfeed.get_with_details()
        logger.info("Detailed domains:")
        pprint(
            detailed_domains[:5] if len(detailed_domains) > 5 else detailed_domains
        )  # Show only first 5
        ...
        if sources and len(sources) > 0:
            ...
            logger.info(
                "This demonstrates the fixed authentication with source parameter"
            )
        ...
        if sources and len(sources) > 0:
            source = sources[0]  # Use the first source as an example
            detailed_source_domains = client.intelfeed.get(
                source=source, fulldetails=True
            )
            logger.info(f"Detailed domains from source '{source}':")
            pprint(
                detailed_source_domains[:5]
                if len(detailed_source_domains) > 5
                else detailed_source_domains
            )
