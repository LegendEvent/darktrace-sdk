...
def get_threat_intelligence(client: DarktraceClient) -> List[Dict[str, Any]]:
    ...
    entries = client.intelfeed.get(source=THREAT_INTEL_SOURCE, fulldetails=True)
...
def find_devices_communicating_with_threats(
    client: DarktraceClient, threats: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
...
def get_model_breaches_for_devices(
    client: DarktraceClient, devices: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
    ...
    start_time = int(
        (datetime.now(timezone.utc) - timedelta(days=DAYS_TO_CHECK)).timestamp() * 1000
    )
    ...
    breaches = client.breaches.get(
        from_time=start_time, devices=",".join(str(did) for did in device_ids)
    )
...
def generate_threat_report(
    ...
    report = {
        ...
        "threats": threats[:10]
        if len(threats) > 10
        else threats,  # Include only first 10 threats
        "affected_devices": [
            {
                "did": device.get("did"),
...
def main():
    ...
    try:
        ...
        logger.info(
            f"- Affected devices: {report['summary']['affected_devices_count']}"
        )
        logger.info(
            f"- Related model breaches: {report['summary']['model_breaches_count']}"
        )
